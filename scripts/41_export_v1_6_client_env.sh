...
# client/robot-dh-v1-6.env（chmod 600），同时 stdout 仍保持脱敏。
#
# 相比 31_argowf_remote_env_export.sh 增加：
...
#   ./scripts/41_export_v1_6_client_env.sh --mode tunnel
#   ./scripts/41_export_v1_6_client_env.sh --host my.public.host

SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
PROJECT_DIR=$(cd -- "$SCRIPT_DIR/.." && pwd)
ENV_FILE="$PROJECT_DIR/.env"
OUTPUT_FILE="$PROJECT_DIR/client/robot-dh-v1-6.env"
...
ROBOT_DH_S3_REGION="${ROBOT_DH_S3_REGION:-us-east-1}"
ARGO_NAMESPACE="${ROBOT_DH_ARGO_NAMESPACE:-robot-dh}"
RELEASE_VERSION="${ROBOT_DH_RELEASE_VERSION:-v1.6}"
PLATFORM_VERSION="${ROBOT_DH_PLATFORM_VERSION:-1.6}"
QC_PREFIX="${ROBOT_DH_QC_CONTRACT_BUCKET_PREFIX:-s3://${ROBOT_DH_LAKE_BUCKET}/qc}"
ML_READY_ROOT="${ROBOT_DH_ML_READY_ROOT:-s3://${ROBOT_DH_LAKE_BUCKET}/ml-ready}"
WORKFLOW_TMP_PREFIX="${ROBOT_DH_WORKFLOW_TMP_PREFIX:-s3://${ROBOT_DH_LAKE_BUCKET}/tmp/workflows}"
...
render_env() {
  ...
  cat <<EOF
# robot-dh v1.6 client env (mode=${MODE})
# 不要把真实密码提交到 git；--show-secrets 生成的文件已 chmod 600。

ROBOT_DH_PLATFORM_VERSION=${PLATFORM_VERSION}
...
ROBOT_DH_S3_REGION=${ROBOT_DH_S3_REGION}
...
# v1.6 新增：QC contract / ML-ready / workflow tmp 根前缀
ROBOT_DH_QC_CONTRACT_BUCKET_PREFIX=${QC_PREFIX}
ROBOT_DH_ML_READY_ROOT=${ML_READY_ROOT}
ROBOT_DH_WORKFLOW_TMP_PREFIX=${WORKFLOW_TMP_PREFIX}
EOF
}
